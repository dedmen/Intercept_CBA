...
def main():
    print("""
  #########################
  # Intercept Debug Build #
  #########################
""")

    scriptpath = os.path.realpath(__file__)
    projectpath = os.path.dirname(os.path.dirname(scriptpath))
    addonspath = os.path.join(projectpath, "rv")
    ...
    skipped = 0
    removed = 0
    for example_dir in os.listdir(addonspath):
        
        example_path = os.path.join(addonspath, example_dir)
        example_path = os.path.join(example_path, "addons")
        ...
        for p in os.listdir(example_path):
            path = os.path.join(example_path, p)
            if not os.path.isdir(path):
                continue
            if p[0] == ".":
                continue
            if not check_for_changes(example_path, p):
                skipped += 1
            ...
            try:
                subprocess.check_output([
                    "makepbo",
                    "-NUP",
                    "-@={}\\{}\\{}\\addons\\{}".format(MAINPREFIX,PREFIX.rstrip("_"),example_dir,p),
                    p,
                    "{}{}.pbo".format(PREFIX,p)
